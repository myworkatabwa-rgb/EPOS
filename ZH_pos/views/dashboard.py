...
def dashboard(request):
    ...
    cash_sale = today_orders.filter(payment_method="cash").aggregate(
        total=Sum("total")
    )["total"] or 0

    card_sale = today_orders.filter(payment_method="card").aggregate(
        total=Sum("total")
    )["total"] or 0

    split_sale = today_orders.filter(payment_method="split").aggregate(
        total=Sum("total")
    )["total"] or 0
